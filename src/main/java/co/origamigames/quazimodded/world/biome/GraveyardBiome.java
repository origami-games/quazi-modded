...
public class GraveyardBiome extends Biome {
    ...
    public GraveyardBiome() {
        super(new Biome.Settings().configureSurfaceBuilder(SurfaceBuilder.DEFAULT, QMSurfaceBuilder.GRAVEYARD_CONFIG)
                ...
                .temperature(.6F).downfall(.7F).waterColor(0).waterFogColor(0).parent(null));
    ...
    static {
        ...
        //         new SimpleBlockStateProvider(QMBlocks.GRAVESTONE.getDefaultState()), new SimpleBlockPlacer())).tries(1)
        //                 .build();

        FANCY_TREE_NO_LEAVES_CONFIG = (new BranchedTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.OAK_LOG.getDefaultState()),
                new SimpleBlockStateProvider(Blocks.AIR.getDefaultState()), new BlobFoliagePlacer(0, 0))).build();
        OAK_TREE_NO_LEAVES_CONFIG = (new BranchedTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.OAK_LOG.getDefaultState()),
                new SimpleBlockStateProvider(Blocks.AIR.getDefaultState()), new BlobFoliagePlacer(2, 0))).baseHeight(4).heightRandA(2)
